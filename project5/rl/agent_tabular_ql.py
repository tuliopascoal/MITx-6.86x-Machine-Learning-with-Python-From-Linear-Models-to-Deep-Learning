...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random_sample() < epsilon: #random coin trigger
        ...
        (action_index,
         obj_index) = np.unravel_index(np.argmax(q_values, axis=None),
                                          q_values.shape)
...
def run_episode(for_training):
    ...
    while not terminal:
        room_id = dict_room_desc[current_room_desc]
        quest_id = dict_quest_desc[current_quest_desc]
        (action_index, object_index) = epsilon_greedy(room_id, quest_id, q_func, epsilon)
        ...
        if for_training:
            ...
            tabular_q_learning(q_func, room_id, quest_id, action_index, object_index, reward, next_room_desc_id, next_quest_desc_id, terminal)

        if not for_training:
            epi_reward = epi_reward + gamma * reward
            gamma *= GAMMA
