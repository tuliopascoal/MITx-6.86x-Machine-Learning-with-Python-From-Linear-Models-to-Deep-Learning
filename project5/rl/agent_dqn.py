...
def epsilon_greedy(state_vector, epsilon):
    ...
    """
    # TODO Your code here
    if np.random.random_sample() < epsilon:  # random coin trigger
        action_index = np.random.randint(NUM_ACTIONS)
        obj_index = np.random.randint(NUM_OBJECTS)
    else:
        with torch.no_grad():
            q_value_decision, q_value_obj = model(state_vector)
            action_index = torch.argmax(q_value_decision)
            obj_index = torch.argmax(q_value_obj)
    return (action_index, obj_index)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # TODO Your code here
    q_value_cur_state_decision = 1 / 2 * (q_value_cur_state[0][action_index]
                                        + q_value_cur_state[1][object_index])

    goal = reward + GAMMA *maxq_next *(1-terminal)
    loss = (q_value_cur_state_decision-goal).pow(2).mean()
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # TODO Your code here
        (action_index, obj_index) = epsilon_greedy(current_state_vector,
                                                      epsilon)
        (next_room_desc, next_quest_desc, reward,
         terminal) = framework.step_game(current_room_desc, current_quest_desc, action_index, obj_index)

        if for_training:
            # update Q-function.
            # TODO Your code here
            nextstate = next_room_desc + next_quest_desc
            nextstate_vec = utils.extract_bow_feature_vector(nextstate, dictionary)
            deep_q_learning(current_state_vector, action_index, obj_index, reward, nextstate_vec, terminal)
