...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random_sample() < epsilon:  # random coin trigger
        ...
        action_index, obj_index = index2tuple(ind)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    """
    # TODO Your code here
    nextq_values = theta @ next_state_vector
    nextmaxq = np.max(nextq_values)
    qvalues = theta @ current_state_vector
    ind = tuple2index(action_index, object_index)
    qvalue_now = qvalues[ind]
    goal = reward + GAMMA * nextmaxq * (1-terminal)

    theta[ind] = theta[ind] + ALPHA * (goal-qvalue_now)*current_state_vector # TODO Your update here
# pragma: coderesponse end
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            ...
            linear_q_learning(theta, current_state_vector, action_index, object_index, reward, nextstate_vec, terminal)
