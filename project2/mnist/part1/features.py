...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    """
    # TODO: first center data using the feature_means (this is done by center_data() function)
    ...
    ### The below version does not consider that argumenst X and feature_means are
    # already centered (in main.py both are sent already centered
    # centered_X = center_data(X)
    # pcs = pcs[:, :n_components]
    # pca_X = np.dot(centered_X, pcs)
    #
    # return pca_X

    ### version considering centered already X and feature_means
    centered_X = X - np.transpose(feature_means)
    pcs = pcs[:, :n_components]
    pca_X = np.dot(centered_X, pcs)
    return pca_X
