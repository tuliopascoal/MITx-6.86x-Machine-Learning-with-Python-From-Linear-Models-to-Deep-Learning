...
def estep(X: np.ndarray, mixture: GaussianMixture) -> Tuple[np.ndarray, float]:
    ...
    for i in range(n):
        ...
        total_sum = logsumexp(post[i, :])
        post[i, :] = post[i, :] - total_sum
        l_old += total_sum

    return np.exp(post), l_old
...
def mstep(X: np.ndarray, post: np.ndarray, mixture: GaussianMixture,
    ...
    for j in range(K):
        ...
        for l in range(d):
            mask = (X[:, l] != 0)
            nsum = post[mask, j].sum()
            if (nsum>=1):
                mu[j, l] = (X[mask, l] @ post[mask, j]) / nsum #new mean
            sse = sse + ((mu[j, l] - X[mask, l]) ** 2) @ post[mask, j]
            weight += nsum
        var[j] = sse / weight
...
def fill_matrix(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    for i in range(n):
        ...
        Xpred[i, mask0] = np.dot(post, mixture.mu[:, mask0])
    return Xpred
    raise NotImplementedError
