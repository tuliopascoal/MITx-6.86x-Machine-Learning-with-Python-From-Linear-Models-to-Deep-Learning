...
# The Average Perceptron achieved better Training accuracy (0.9728), and also
# a better Validation accuracy (0.7980).
# Therefore, we can conclude that it was the best classifier. Now, we need to use
# it using its optimal parameter, i.e., T = 25.000.

#Computing the testing accuracy on the test set
T = 25.0000
L = 0.0100

avg_percep_train_accuracy, avg_percep_test_accuracy = p1.classifier_accuracy(p1.average_perceptron, train_bow_features, test_bow_features, train_labels, test_labels, T=T)
print('making prediction using Average Perceptron, with T =', T)
print("{:43} {:.4f}".format("Training accuracy for AVG PERCEP:", avg_percep_train_accuracy))
print("{:43} {:.4f}".format("Testing  accuracy for AVG PERCEP:", avg_percep_test_accuracy))

peg_train_accuracy, peg_test_accuracy = p1.classifier_accuracy(p1.pegasos, train_bow_features, test_bow_features, train_labels, test_labels, T=T, L=L)
print('making prediction using Pegasos, with T =', T, 'and L =', L)
